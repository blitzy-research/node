...
const { isAbsolute, join, resolve, relative, sep } = require('path');
...
const { kDefaultPattern } = require('internal/test_runner/utils');
...
const kNodeModulesPrefix = `node_modules${sep}`;
const kNodeModulesSegment = `${sep}node_modules${sep}`;
...
class TestCoverage {
  ...
  shouldSkipFileCoverage(url) {
    ...
    // relative() cannot express a file that lives on another Windows volume as a
    // working directory relative path, so it returns an absolute path such as
    // 'D:\test\project\src.js' instead. The built-in exclusions below only ever
    // decide on a genuinely relative path, because matching them against an
    // absolute one would let the ancestor directory names of that other volume
    // empty the coverage report again. User supplied globs keep matching absolute
    // paths, which is the documented behaviour of --test-coverage-exclude.
    const isRelativeToCwd = !isAbsolute(relativePath);
    const {
      coverageExcludeGlobs: excludeGlobs,
      coverageIncludeGlobs: includeGlobs,
    } = this.options;

    // This check filters out files that match the exclude globs. User supplied
    // globs are documented to match both absolute and relative paths, but the
    // built-in default pattern is not user supplied: it is the very same pattern
    // that test discovery resolves relative to the working directory (see
    // createTestFileList() in internal/test_runner/runner.js). Matching its
    // 'test/**/*' alternative against the absolute path made every file of a
    // project stored below a directory named 'test' (for example
    // /home/test/my-project) look like a test file, which silently emptied the
    // coverage report. Refs: https://github.com/nodejs/node/issues/58654
    if (excludeGlobs?.length > 0) {
      for (let i = 0; i < excludeGlobs.length; ++i) {
        const excludeGlob = excludeGlobs[i];

        // Skip the built-in default for files that have no working directory
        // relative path at all, so that it can never match an absolute one.
        if (excludeGlob === kDefaultPattern && !isRelativeToCwd) continue;

        if (matchGlobPattern(relativePath, excludeGlob) ||
            (excludeGlob !== kDefaultPattern &&
             matchGlobPattern(absolutePath, excludeGlob))) {
          return true;
        }
      }
    }
    ...
    // This check filters out the node_modules/ directory, unless it is explicitly
    // included. Like the default exclusion above, the check runs on the working
    // directory relative path so that a project stored below a directory named
    // node_modules is not excluded in its entirety. This mirrors test discovery,
    // which skips node_modules by directory name. A file that has no working
    // directory relative path cannot be classified this way, so it is reported.
    return isRelativeToCwd &&
           (StringPrototypeStartsWith(relativePath, kNodeModulesPrefix) ||
            StringPrototypeIncludes(relativePath, kNodeModulesSegment));
  }
}
